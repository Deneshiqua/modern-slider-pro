...
const Canvas = () => {
  ...
  /** Align slide frame center with the visible scrollport (not scroll-range midpoint / layout overflow). */
  const centerFrameInViewport = useCallback(() => {
    const scrollEl = scrollRef.current;
    const frameEl = frameRef.current;
    if (!scrollEl || !frameEl) return;
    ...
    // clientWidth/Height = visible scrollport; bounding rect can include overflow under siblings (e.g. timeline).
    const scrollCenterX = scrollRect.left + scrollEl.clientWidth / 2;
    const scrollCenterY = scrollRect.top + scrollEl.clientHeight / 2;
    const frameCenterX = frameRect.left + frameRect.width / 2;
    const frameCenterY = frameRect.top + frameRect.height / 2;

    scrollEl.scrollLeft += frameCenterX - scrollCenterX;
    scrollEl.scrollTop += frameCenterY - scrollCenterY;
  }, []);
  ...
  const fitCanvasToViewport = useCallback(() => {
    ...
    if (fitZ !== canvasZoomRef.current) {
      flushSync(() => {
        setCanvasZoom(fitZ);
      });
      canvasZoomRef.current = fitZ;
    }

    // Shell min-size / pan gutters update after zoom — wait for layout before centering.
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        centerFrameInViewport();
      });
    });
  }, [canvasSettings, clampZoom, centerFrameInViewport, setCanvasZoom, viewMode]);

  useEffect(() => {
    registerCanvasViewport({
      centerView: centerFrameInViewport,
      fitToViewport: fitCanvasToViewport,
    });
    return () => registerCanvasViewport(null);
  }, [registerCanvasViewport, centerFrameInViewport, fitCanvasToViewport]);
  ...
  }, [cancelRulerPointerRaf, canvasSettings.showRulers]);
  ...
  useLayoutEffect(() => {
    ...
    const keyDims = `${viewMode}:${viewport.width}x${viewport.height}:${cw}x${ch}`;
    ...
    queueMicrotask(() => {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => centerFrameInViewport());
      });
    });
  }, [viewMode, viewport.width, viewport.height, scrollClient.w, scrollClient.h, centerFrameInViewport]);
  ...
  }, [clampZoom, isCanvasPreview, zoomTowardScreenPoint]);

  // Middle-mouse drag to pan the canvas viewport (Photoshop-style).
  useEffect(() => {
    const scrollElement = scrollRef.current;
    if (!scrollElement || isCanvasPreview) return;
  ...
  const navigateToSlide = useCallback(
    (index: number, direction?: SlideTransitionDirection) => {
      ...
      setSlideTransitionDirection(direction ?? (clamped >= currentSlideIndex ? 1 : -1));
  ...
  const goPrev = () => {
    const prev = settings.loop
      ? (currentSlideIndex - 1 + slides.length) % slides.length
      : Math.max(currentSlideIndex - 1, 0);
    navigateToSlide(prev, -1);
  };
  ...
  const showChromeRulers = canvasSettings.showRulers;
  const rulerMetrics = {
    ...rulerViewport,
    canvasZoom,
  ...
  const editorChrome = (
    <>
      ...
      {!isCanvasPreview && canvasSettings.snapToElements && snapGuides.x.map((x, i) => (
        <div
          key={`sg-x-${i}`}
      ...
      {!isCanvasPreview && canvasSettings.snapToElements && snapGuides.y.map((y, i) => (
        <div
          key={`sg-y-${i}`}
      ...
      {!isCanvasPreview && canvasSettings.showCenterGuides && (
        <>
          <div
  ...
  return (
    <div className="msp-relative msp-flex msp-h-full msp-min-h-0 msp-w-full msp-flex-col msp-bg-muted msp-overflow-hidden">
      <div
      ...
      {!isCanvasPreview && (
        <div className="msp-pointer-events-none msp-absolute msp-inset-x-3 msp-bottom-3 msp-z-50 msp-flex msp-max-w-full msp-flex-wrap msp-items-center msp-justify-end msp-gap-1">
          <div
            className="msp-pointer-events-auto msp-shrink-0 msp-rounded-md msp-border msp-bg-card/95 msp-px-2 msp-py-1 msp-text-[10px] msp-font-medium msp-tabular-nums msp-text-muted-foreground msp-shadow-md msp-backdrop-blur-sm"
            title={t('editor.canvas.slideSize')}
          >
            {viewport.width} × {viewport.height}
          </div>
          <div className="msp-pointer-events-auto msp-flex msp-max-w-full msp-flex-wrap msp-items-center msp-gap-0.5 msp-rounded-md msp-border msp-bg-card/95 msp-p-0.5 msp-shadow-md msp-backdrop-blur-sm">
            <Button
              type="button"
              variant="ghost"
              size="icon"
            ...
            <div className="msp-mx-0.5 msp-h-5 msp-w-px msp-shrink-0 msp-bg-border" aria-hidden />
            <Button
              type="button"
              variant="ghost"
              size="icon"
